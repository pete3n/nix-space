# Home-manager module register for Darwin specific modules.
#
...
let
  ...
  tagModules = {
    ...
    git = [ ./git ];
    gpg-user = [ ./gpg ];
    laptop = [ ./darwin/laptop ];
    media-creation = [ ./media-creation ];
    messaging = [ ./messaging ];
    nixvim = [ ./nixvim ];
    p22 = [ ./p22 ];
    yubi-u2f = [ ./darwin/yubikey/u2f ];
  ...
  multiTagModules = [
    {
      ...
      modules = [
        ./darwin/wm/waybar/mpd.nix
        ./darwin/wm/hyprland/desktop/mpd-visualizer.nix
      ];
    }
    {
      tags = [
      ...
      modules = [ ./darwin/wm/hyprland/hyprlidmon.nix ];
    }
  ];
...
lib.throwIf (bogusTags != [ ])
  "home-modules/darwin.nix: tag gate(s) reference unregistered tag(s): ${lib.concatStringsSep ", " bogusTags} (typo, or missing from tag-registry.nix)"
  (
    lib.warnIf (unhandledTags != [ ])
      "home-modules/darwin.nix: host declares tag(s) with no home module: ${lib.concatStringsSep ", " unhandledTags}"
      {
        imports = [
          # Always present: identity, stateVersion, XDG, and the Darwin
          # session variables that differ because the OS is Darwin rather
