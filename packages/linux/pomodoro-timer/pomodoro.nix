...
#
# The config TUI gets its terminal and class from a wrapper this module
# installs, because the package emits the bare TUI and leaves display to the
# caller. That is deliberate: a launcher that opens its own terminal cannot be
# composed, which is what forced title matching before.
{
  config,
  lib,
  pkgs,
...
let
  ...
  #
  # pomodoro-config is the TUI itself — python calling curses — and opens no
  # window of its own. That is a change from the package's earlier form, which
  # launched alacritty internally with a fixed title and dimensions: wrapping
  # THAT produced two windows, an empty one matching the rule and the real UI
  # tiled beside it.
  #
  # Wrapping here means the terminal follows
  # nixSpace.programs.terminals.primary and the window carries a CLASS, so the
  # rule no longer has to match on title and catch every other window called
  # "Pomodoro".
  configLauncher =
...
in
{
  options.nixSpace.hyprdesktop.pomodoro = {
    enable = lib.mkEnableOption "pomodoro timer widget" // {
      default = true;
      description = ''
        Pomodoro timer, its bar clock, and its window rules.

        Disabling it removes the CLOCK too — the ticker renders both. Set
        this false only if something else provides one.
      '';
    };

    package = lib.mkOption {
      type = lib.types.package;
      default = pkgs.pomodoro-timer;
      defaultText = lib.literalExpression "pkgs.pomodoro-timer";
      description = ''
